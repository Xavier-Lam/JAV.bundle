...
class HTTP11Connection(object):
    ...
    def connect(self):
        ...
        if self._sock is None:
            ...
            if self.secure:
                if self.proxy_host:
                    # Send CONNECT request to proxy (without BufferedSocket yet)
                    connect_req = "CONNECT %s:%s HTTP/1.1\r\nHost: %s:%s\r\n\r\n" % (
                        self.host, self.port, self.host, self.port
                    )
                    sock.sendall(connect_req.encode('latin-1'))
                    
                    # Set a timeout for reading proxy response
                    sock.settimeout(10)
                    
                    # Read proxy response - read until we get HTTP response line
                    response_data = bytearray()
                    while len(response_data) < 4096:  # Max 4KB for headers
                        try:
                            chunk = sock.recv(1024)
                            if not chunk:
                                break
                            response_data.extend(chunk)
                            # Check if we've received full headers (ends with \r\n\r\n)
                            if b'\r\n\r\n' in bytes(response_data):
                                break
                        except socket.timeout:
                            break
                    
                    # Check if proxy connection succeeded (look for "200" in response)
                    response_str = bytes(response_data).decode('latin-1', errors='ignore')
                    if '200' not in response_str.split('\r\n')[0]:
                        raise Exception("Proxy connection failed: " + response_str.split('\r\n')[0])
